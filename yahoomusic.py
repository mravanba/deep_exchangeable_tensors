...
def load_data(p_validation = 0.07):
    ...
    ratings_one_hot = one_hot(ratings, False)


    yahoo = {}
    yahoo["mat_values_all"] = ratings
    yahoo["mat_values_tr"] = ratings[mask_tr_val_split == 0]
    yahoo["mat_values_val"] = ratings[mask_tr_val_split == 1]
    yahoo["mat_values_test"] = ratings[mask_tr_val_split == 2]
    yahoo["mat_values_tr_val"] = ratings[mask_tr_val_split <= 1]

    yahoo["mat_values_all_one_hot"] = ratings_one_hot
    yahoo["mat_values_tr_one_hot"] = ratings_one_hot[mask_tr_val_split == 0, :]
    yahoo["mat_values_val_one_hot"] = ratings_one_hot[mask_tr_val_split == 1, :]
    yahoo["mat_values_test_one_hot"] = ratings_one_hot[mask_tr_val_split == 2, :]
    yahoo["mat_values_tr_val_one_hot"] = ratings_one_hot[mask_tr_val_split <= 1, :]

    yahoo["mask_indices_all"] = index
    yahoo["mask_indices_tr"] = index[mask_tr_val_split == 0, :]
    yahoo["mask_indices_val"] = index[mask_tr_val_split == 1, :]
    yahoo["mask_indices_test"] = index[mask_tr_val_split == 2, :]
    yahoo["mask_indices_tr_val"] = index[mask_tr_val_split <= 1, :]

    yahoo["mask_tr_val_split"] = mask_tr_val_split

    n, m = index.max(axis=0) + 1
    yahoo['mat_shape'] = (n, m, np.unique(ratings).shape[0])
    return yahoo
            ...
            for ep in range(opts.get('restore_point_epoch', 0), opts['epochs'] + opts.get('restore_point_epoch', 0)):
                ...
                if 'by_row_column_density' in opts['sample_mode']:
                    ...
                    for sample_ in tqdm(sample_dense_values_uniform(data['mask_indices_tr'], minibatch_size, iters_per_epoch), 
                        ...
                        mat_values = data['mat_values_tr'][sample_] if lossfn == "mse" else data['mat_values_tr_one_hot'][sample_].flatten()
                        mask_indices = data['mask_indices_tr'][sample_]

                        tr_dict = {mat_values_tr:mat_values.flatten(),
                                    mask_indices_tr:mask_indices,
                                    mask_split:np.ones_like(mat_values)
                                    }
                    ...
                    for _, _, _, _, sample_ in tqdm(conditional_sample_sparse(data['mask_indices_tr'], 
                                                                              ...
                                                                              [N,M,1], maxN, maxM), 
                                                    total=iters_per_epoch):
                        mat_values = data['mat_values_tr'][sample_] if lossfn == "mse" else data['mat_values_tr_one_hot'][sample_]
                        mask_indices = data['mask_indices_tr'][sample_]
                ...
                if (ep+1) % opts['validate_interval'] == 0: # Validate and test every validate_interval epochs
                    ...
                    vals = data['mat_values_tr'] if lossfn =="mse" else data['mat_values_tr_one_hot'].flatten()
                    val_dict = {mat_values_tr:vals.flatten(),
                    ...
                    ## Test Loss     
                    test_dict = {mat_values_tr:vals.flatten(),
                                mask_indices_tr:data['mask_indices_tr'],
                                mat_values_val:val_ratings.flatten(),
                                mask_indices_val:data['mask_indices_all'],
                                mask_indices_tr_val:data['mask_indices_all'],
    ...
    opts ={'epochs': 6000,#never-mind this. We have to implement look-ahead to report the best result.
           ...
           'encoder':[
            ...
            'decoder':[
               ...
               {'type':'matrix_sparse', 'units':units},
               {'type':'channel_dropout_sparse'},
               {'type':'matrix_sparse', 'units':71 if lossfn == "ce" else 1, 'activation':None},
            ],
