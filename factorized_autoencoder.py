...
def main(opts):
    ...
    with tf.Graph().as_default():
        ...
        # For validation, since we need less memory (forward pass only), we are feeding the whole matrix. 
        # This is only feasible for this smaller dataset. 
        ...
        for ep in range(opts['epochs']):
            ...
            for indn_, indm_ in tqdm(sample_submatrix(data['mask_tr'], maxN, maxM), total=iters_per_epoch):#go over mini-batches
                inds_ = np.ix_(indn_,indm_,range(num_features))#select a sub-matrix given random indices for users/movies
            ...
            if opts.get("loss", "mse") == "mse":
                ...
                mse_tf_valid, = sess.run([mse_loss_valid], feed_dict=val_dict)
                loss_val_ = np.sqrt(mse_tf_valid)
           ...
           'encoder':[
            ...
            'defaults':{#default values for each layer type (see layer.py)s
                'matrix_dense':{
                    ...
                    'activation': lambda x: x * tf.nn.sigmoid(x), # Swish
