...
from tensorflow.contrib.framework import model_variable
...
def main(opts):        
    ...
    with tf.Graph().as_default():
        # with tf.device('/gpu:0'):
            ...
            with tf.variable_scope(None, default_name="input_features",
                               initializer=opts['defaults']['matrix_sparse'].get('kernel_initializer', None),
                               regularizer=opts['defaults']['matrix_sparse'].get('regularizer', None),
                               reuse=False,):
                mvec_feat = model_variable("mvec_feat",shape=[1,M,1],trainable=True)
                nvec_feat = model_variable("nvec_feat",shape=[N,1,1],trainable=True)
            ...
            with tf.variable_scope("decoder"):
                tr_dict = {#'nvec':out_enc_tr['nvec'],
                           ...
                           'input':masked_inner_product(out_enc_tr['nvec'], out_enc_tr['mvec'], mask_indices_tr),
                           'mask_indices':mask_indices_tr,
                           'units':1,#out_enc_tr['units'],
                           'shape':out_enc_tr['shape']}
                val_dict = {#'nvec':out_enc_val['nvec'],
                            #'mvec':out_enc_val['mvec'],
                            'input':masked_inner_product(out_enc_val['nvec'], out_enc_val['mvec'], mask_indices_tr_val),
                            'mask_indices':mask_indices_tr_val,
                            'units':1,#out_enc_val['units'],
...
if __name__ == "__main__":
    ...
    opts ={'epochs': 20000,#never-mind this. We have to implement look-ahead to report the best result.
...
#               {'type':'matrix_dropout_sparse'},
            ...
            'defaults':{#default values for each layer type (see layer.py)
                'matrix_sparse':{
                    ...
                    'pool_mode':'sum',#mean vs max in the exchangeable layer. Currently, when the mask is present, only mean is supported
                    #'kernel_initializer': tf.contrib.layers.xavier_initializer(),#tf.random_normal_initializer(0, .01),
