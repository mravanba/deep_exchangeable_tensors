...
from util import get_data, get_movielens_sparse
    ...
    with tf.Graph().as_default():
        # with tf.device('/gpu:0'):
            ...
            for ep in range(opts['epochs']):
                ...
                iters_per_epoch = 1#data['mask_indices_tr'].shape[0] / 10
                minibatch_size = data['mask_indices_tr'].shape[0] / iters_per_epoch
                ...
                print("Before: %f" % loss_tr_)

                _, X = np.unique(data['mask_indices_tr'][:,0], return_inverse=True)
                _, Y = np.unique(data['mask_indices_tr'][:,1], return_inverse=True)
                rescaled_indices = np.array(list(zip(X,Y)))
                batchN = rescaled_indices[-1,0]
                batchM = np.max(rescaled_indices[:,1])

                tr_dict = {mat_values_tr:data['mat_values_tr'],
                                mask_indices_tr:rescaled_indices,
                                mat_shape_tr:[batchN+1,batchM+1,1]}


                loss_tr_, rec_loss_tr_, val_values = sess.run([total_loss, rec_loss, out_val], feed_dict=tr_dict)
                print(np.sqrt(np.mean(np.square(data["mat_values_val"] - val_values))))
                ## Validation Loss
